...
export class NumberAddingNegativeNumbers implements Question {
  public order;
  public answerLabel = '%1d';
  public units = '';
  public questionLabel = ' %1$d + (%2$d) =';
  public inputParams: number[];
  public answers: any[];
  public type = QuestionTypes.NumberAddNegativeNumbers;
  ...
  correctAnswer(): number {
      return this.inputParams[0] + this.inputParams[1];
  }

  wrongAnswer(mode: number): number {

      switch (mode) {
          case 1: return this.inputParams[1] - this.inputParams[0];
          case 2: return Math.floor(Math.random() * 20) + Math.floor(Math.random() * 20) - 20;
      }
  }
